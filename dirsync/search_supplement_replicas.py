...
def _search_replicas(nr_replicas, filter_out=None):
    ...
    if invocation_counter > 1:
        print "Don't invoke this function more than once"
        sys.exit(1)
    if filter_out is None:
        filter_out = list()

    if nr_replicas == 1:
        print "Missing 1 replicas from arguments. Searching for it in the filesystem..."
    elif nr_replicas == 2:
        print "Missing 2 replicas from arguments. Searching for them in the filesystem..."
    candidates = list()
    for parentdir in config.POSSIBLE_PARENT_DIRS:
        print "\tSearching under '%s'..." % (parentdir,)
...
def _validate_enough_replicas(candidates, nr_replicas):
    if len(candidates) < nr_replicas:
        if len(candidates) == 0:
            print "Did not find any replica in the filesystem to supplement existing."
        elif len(candidates) == 1:
            print "Did not find enough replicas in the filesystem to to pick from."
            print "Only the following was found: %s" % (candidates[0],)
        else:
            assert False
        sys.exit(1)
...
def print_dirs(dirs):
    ...
    for dir_idx, _dir in enumerate(dirs):
        ...
        print "%d: \t%s\t%s\t%s" % (dir_idx, _dir, device_size, device_info_output)
