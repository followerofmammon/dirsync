...
def _search_replicas(nr_replicas, filter_out=None):
    ...
    if invocation_counter > 1:
        printer.print_string("Don't invoke this function more than once")
        sys.exit(1)
    if filter_out is None:
        filter_out = list()

    if nr_replicas == 1:
        printer.print_string("Missing 1 replicas from arguments. Searching for it in the filesystem...")
    elif nr_replicas == 2:
        printer.print_string("Missing 2 replicas from arguments. Searching for them in the filesystem...")
    candidates = list()
    for parentdir in config.POSSIBLE_PARENT_DIRS:
        printer.print_string("\tSearching under '%s'..." % (parentdir,))
...
def _validate_enough_replicas(candidates, nr_replicas):
    if len(candidates) < nr_replicas:
        if len(candidates) == 0:
            printer.print_string("Did not find any replica in the filesystem to supplement existing.")
        elif len(candidates) == 1:
            printer.print_string("Did not find enough replicas in the filesystem to to pick from.")
            printer.print_string("Only the following was found: %s" % (candidates[0],))
        else:
            assert False
        sys.exit(1)
...
def print_dirs(dirs):
    printer.print_string("\tReading device list...")
    ...
    for dir_idx, _dir in enumerate(dirs):
        ...
        printer.print_string("%d: \t%s\t%s\t%s" % (dir_idx, _dir, device_size, device_info_output))
